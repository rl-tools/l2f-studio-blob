...
class GRULayer{
    ...
    evaluate(input){
        if(input.size().length === 2){
            ...
            console.assert(INPUT_SIZE > 1, "Input size needs to be larger than 1 because of math.sequeeze")
            this.reset()
            const outputs = []
            for(let i = 0; i < SEQUENCE_LENGTH; i++){
                outputs.push(this.evaluate_step(math.squeeze(math.subset(input, math.index(i, math.range(0, BATCH_SIZE), math.range(0, INPUT_SIZE))))))
            }
